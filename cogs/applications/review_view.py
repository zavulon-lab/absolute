...
# ===== ИМПОРТ ЛОГИРОВАНИЯ И БАЗЫ =====
from database import log_staff_action

# !!! ВАЖНО: Тебе нужно добавить эту функцию в database.py (см. ниже)
# from database import set_application_cooldown 
# Пока я сделаю заглушку, если функции нет, но лучше импортировать её
try:
    from database import set_application_cooldown
except ImportError:
    # Заглушка, если функции еще нет
    def set_application_cooldown(user_id, days):
        pass
...
class ApplicationReviewView(View):
    ...
    async def process_denial(self, interaction: Interaction, member: disnake.Member, reason: str):
        ...
        # 🧊 УСТАНАВЛИВАЕМ КУЛДАУН НА 7 ДНЕЙ
        # Эта функция должна записать в БД дату, до которой юзеру нельзя подавать заявку
        try:
            set_application_cooldown(member.id, days=7)
        except Exception as e:
            print(f"[Error] Не удалось установить кулдаун: {e}")
        ...
        # Вычисляем дату разбана для сообщения
        unban_date = datetime.now() + timedelta(days=7)
        unban_timestamp = int(unban_date.timestamp())

        result_embed = Embed(
            description=(
                f"Заявка от пользователя {member.mention}\n\n"
                f"На Вступление в семью была отклонена. <:cross:1473380950770716836>\n\n"
                f"Причина: **{reason}**\n"
                f"Следующая подача возможна: <t:{unban_timestamp}:D> (<t:{unban_timestamp}:R>)\n\n"
                f"Рассматривал заявку: {recruiter.mention}"
            ),
            color=0xED4245,
        )
        result_embed.set_thumbnail(url=member.display_avatar.url)
        result_embed.set_footer(text="Absolute Famq", icon_url=interaction.client.user.display_avatar.url)
        ...
        await interaction.followup.send(f"<:cross:1473380950770716836> Заявка {member.mention} отклонена (КД 7 дней).", ephemeral=True)
    ...
    async def process_acceptance_final(self, interaction: Interaction, member: disnake.Member):
        ...
        if original_embed:
            ...
            original_embed.add_field(name="▬▬▬▬▬▬▬▬▬▬", value="**<:tick:1473380953245221016> ПРИНЯТ**", inline=False)
            original_embed.add_field(
                name="<:freeiconmanager7887021:1473386161329148086> Куратор", value=curator.mention, inline=True
            )
            original_embed.add_field(
                name="<:freeiconrecruiter7250697:1473386162566598756> Рекрутер", value=recruiter.mention, inline=True
            )
        ...
        try:
            ...
            if academy_channel:
                academy_embed = Embed(
                    title="<:add:1473387233372541102> Новый участник принят",
                    description=(
        ...
        await interaction.followup.send(
            f"<:tick:1473380953245221016> {member.mention} принят. Вы назначены куратором.", ephemeral=True
        )

    # ========== КНОПКИ ==========
    @button(label=" Принять (После обзвона)", style=ButtonStyle.success, custom_id="app_accept", emoji="<:tick:1473380953245221016>")
    ...
    @button(label="Взять на рассмотрение", style=ButtonStyle.secondary, custom_id="app_review", emoji= "<:ddd:1473378828427460618>")
    ...
    @button(label=" Вызвать на обзвон", style=ButtonStyle.primary, custom_id="app_call", emoji="<:call:1473378348863586304>")
    async def call_button(self, button: Button, interaction: Interaction):
        ...
        result_embed = Embed(
            description=(
                f"Заявка от пользователя {member.mention}\n\n"
                f"На Вступление в семью была рассмотрена! <:tick:1473380953245221016>\n\n"
    ...
    @button(label="Отклонить", style=ButtonStyle.danger, custom_id="app_deny", emoji="<:cross:1473380950770716836>")
    ...
    @button(label="Создать чат", style=ButtonStyle.secondary, custom_id="app_create_chat", emoji="<:__:1473379222432256083>")
    async def create_chat_button(self, button: Button, interaction: Interaction):
        ...
        try:
            ...
            chat_embed = Embed(
                title="<:teg:1473384504537383157> Обсуждение заявки",
            ...
            dm_embed = Embed(
                title="<:__:1473379222432256083> Уточнение по заявке",
